...
for(var i = 0; i < lockCount; i += 1){
    if lock[i,0] == color_MASTER{//Holy crap, cropping the lock sprites came in clutch
        ...
        for(var i = 0; i < tempH/64; i++) {
            draw_sprite_ext(sprDDynaTexture,floor(global.goldIndex),tempX,tempY+i*64,tempW/64,1,0,c_white,1);
        }
        draw_sprite_part_ext(sprDDynaTexture,floor(global.goldIndex),tempX,tempY+i*64,tempW,tempH-i*64,tempX,tempY+i*64,tempW/64,1,c_white,1)
